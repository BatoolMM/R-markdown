...
library(tidyverse)
#load data
data <- read_csv("../paperToRmd/data/figure-1-data.csv", col_types="fi")
```
## What is Knitr?
...
**Output:** "a total of `r sum(data$count)` individuals from 21 countries completed this section."
...
>> ```
>> Note: since this we just added inline code to calculate the error margin, we can just add this snippet to count the total respondents, this means we only need to substitute `sum(data$count)` for 243 (and don't need to add the hashes and `r` a second time.
>> ```{r eval = FALSE, echo = TRUE}
...
We can actually make our lives easier in one other way too. So far we've loaded the library we need in each code chunk, along with loading the data in each code chunk individually. Instead we can do this all at the beginning. This also makes it easier for us to see all the libraries we need to use in any given document and if we need to tweak our data files at all we don't need to search through every code chunk. 
