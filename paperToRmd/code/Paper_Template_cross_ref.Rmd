...
knit: (function(inputFile, encoding) { 
      ...
      rmarkdown::render(inputFile,
                        encoding=encoding, 
                        output_file=file.path(dirname(inputFile), out_dir, 'Paper_Template_cross_ref.html')) })
---
...
#tidyverse for plots
library(tidyverse)
#knitr to use kable() for table 1
library(knitr)
...
>_"If I have seen further it is because I have stood on the shoulders of giants"_ <br>
> (Isaac Newton)
...
A recent survey conducted by Springer [-@springer_reality_2016] reported that 90% of participants identified "more robust experimental design" as one of several key improvements needed for the conduct of better science, in addition to "better statistics" and "better mentorship". More recently, Nature [-@nature_nature_2018] published the survey data which can be found on Figshare.

A manifesto for reproducible science made several recommendations and called attention to initiatives such as the **Transparency and Openness Promotion** (TOP) guidelines created by the [Center for Open Science](https://www.cos.io/) to improve research planning and reporting [@munafo_manifesto_2017], which was supported by over 5000 journals and research organizations. 

On June 9, 2015, the U.S. National Institutes of Health (NIH) published a notice^1^ that identified four areas for improvement that are now required to be addressed by scientists in grant applications, as follows:
...
we compute that at a 95% confidence level (i.e., $Z=1.96$), with base probability $p=1/2$ and sample size $n=`r sum(data1$count)`$, the margin of error is ±`r round(1.96*0.5*(1-0.5)/sqrt(sum(data1$count))*100)`%.
...
A total of `r sum(data1$count)` individuals from 21 countries completed this section. The majority of the survey participants are core facility directors or managers (69%) and work in an academic setting (72%) in the United States (79%).

## Current Landscape for Rigor and Transparency in Represented Shared Resources

When asked how knowledgeable participants were with respect to the NIH research rigor initiatives, 47% stated they were very familiar, whereas the rest were equally either somewhat familiar or completely unaware of such guidelines (Fig. 3.1).

```{r fig-1, fig.cap="Knowledge and awareness of the current NIH guidelines on rigor and reproducibility."}
pie_data <- data1 %>%
...
Table 3.1 represents the perceived challenges faced in promoting best practices for rigorous research in core settings. Issues associated with sample quality or quantity top the list. 

```{r table-1, fig.align="center"}
# use kable to create better table
kable(table1, caption="Major challenges to rigor observed in shared resources")

```

<br>
Just over 70% of respondents noted that their clients do not routinely request specific information (documentation or practice statements) related to the procedures used by the cores to foster rigorous and reproducible research (Fig. 3.22).

```{r fig-2, fig.cap="Lack of requests for rigor and reproducibility documentation by users of shared resources."}
#add Figure 2 to a code chunk to control alignment and size
...
A second set of multiple-choice questions asked participants to select the new tools they think would enhance or facilitate the implementation of R&R best practices within their core operation (Fig. 3.3).

```{r fig-3, fig.cap="Types of tools that cores would like to implement in their operations." }
data3 %>%
