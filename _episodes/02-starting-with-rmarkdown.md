...
**data:** this folder contains our raw data files. We downloaded both an .xlxs and .txt  FIXME - clean up the data folder
...
FIXME - need to add the following 3 files (or we could skip these):
...
3) Navigate to the `docs` folder and open `paper_raw.txt FIXME`. Copy all the text with either `ctrl-a` or `cmd-a` and paste AFTER the yaml header in our rmd file.
~~~
...
output:
  html_document
...
FIXME fix screenshot when final raw text is determined (just a few sentences will be different)
