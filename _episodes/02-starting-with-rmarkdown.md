...
Do this on your own with your new rmd file:

1) Delete everything except the yaml header 

2) Edit the yaml header to add ... FIXME

3) Copy all the text from the `rawtext.txt FIXME`found in the `docs` folder (`ctrl-a` or `cmd-a`) and paste after the yaml header in our rmd file.
*Obviously this isn't the same process as writing a paper, but we're not going to force you to write the narrative in this workshop, we assume you have plenty of writing to do on your own time.
